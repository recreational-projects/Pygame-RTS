...
def draw(surface_: pg.Surface) -> None:
    ...
    for building in global_buildings:
        if building.health > 0 and (building.team == gdi or building.is_explored):
            building.draw(surface=surface_, camera=camera)

    fog_of_war.draw(surface=surface_, camera=camera)
    for unit in global_units:
        if unit.team == gdi or fog_of_war.is_visible(unit.position):
            unit.draw(surface=surface_, camera=camera)

    for projectile in projectiles:
        if projectile.team == gdi or fog_of_war.is_visible(projectile.position):
    ...
    interface.draw(
        surface_=surface_,
        own_buildings=[b for b in global_buildings if b.team == gdi],
...
class ProductionInterface:
    ...
    def _draw_pending_building(
        ...
        *,
        ...
        temp_surface.fill(self.hq.team.color)
...
if __name__ == "__main__":
    ...
    gdi = Team("GDI")
    nod = Team("NOD")
    gdi_hq = Headquarters(position=(300, 300), team=gdi, font=base_font)
    nod_hq = Headquarters(
        position=(MAP_WIDTH - 300, MAP_HEIGHT - 300), team=nod, font=base_font
    )
    nod_hq.iron = 1500
    ...
    player_units.add(Harvester((400, 400), gdi, gdi_hq, font=base_font))

    ai_units.add(Infantry((2050, 1200), nod))
    ai_units.add(Infantry((2070, 1200), nod))
    ai_units.add(Infantry((2090, 1200), nod))
    ai_units.add(Harvester((2200, 1300), nod, nod_hq, font=base_font))
    ...
    while running:
                    ...
                    if gdi_hq.pending_building:
                        ...
                        if gdi_hq.production_queue and gdi_hq.has_enough_power:
                            gdi_hq.production_timer = gdi_hq.get_production_time(
                                ...
                                friendly_buildings=[
                                    b for b in global_buildings if b.team == gdi
                                ],
                            )
                        continue
                    clicked_field = next(
        ...
        for building in global_buildings:
            if isinstance(building, Headquarters):
                if building.team.name == gdi:
                    building.update(
                        particles=particles,
                        friendly_units=player_units,
                        friendly_buildings=[
                            b for b in global_buildings if b.team.name == "GDI"
                        ],
        ...
        player_buildings = [b for b in global_buildings if b.team.name == "GDI"]
        ai.update(
            friendly_units=ai_units.sprites(),
            friendly_buildings=[b for b in global_buildings if b.team.name != "GDI"],
        ...
        ai_buildings = [b for b in global_buildings if b.team.name == "GDI"]
