...
class GameManagerIso:
    ...
    def run_game(self) -> None:
        ...
        while self.running and self.state == GameState.PLAYING:
                ...
                if event.type == pg.QUIT:
                    ...
                    if event.button == 1:
                        ...
                        if clicked_building:
                            ...
                            if g["player_hq"] is None:
                                raise ValueError("`player_hq` cannot be `None`")
            ...
            for unit in [u for u in unit_list if not u.is_building]:
                unit.update(
                    particles=g["particles"], global_buildings=list(g["global_buildings"]), projectiles=g["projectiles"]
                )
            ...
            draw_allies_mini = frozenset(g["teams"]) if g.get("spectator", False) else g["player_allies"]
            draw_mini_map(
                self.screen,
                g["camera"],
                g["fog_of_war"],
    ...
    def initialize_game(self, *, game_mode: str, size_name: str, map_name: str, spectator_mode: bool = False) -> None:
        ...
        player_units: pg.sprite.Group[UnitIso] = pg.sprite.Group()
